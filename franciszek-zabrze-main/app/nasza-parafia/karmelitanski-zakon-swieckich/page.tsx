...
// Database
import findKarmelitanskiZakonSwieckichData from '@/actions/findKarmelitanskiZakonSwieckichData';
...
export default function KarmelitanskiZakonSwieckich() {
	const [karmelitanskiZakonSwieckich, setKarmelitanskiZakonSwieckich] =
	...
	useEffect(() => {
		const fetchKarmelitanskiZakonSwieckich = async () => {
			try {
				const data = await findKarmelitanskiZakonSwieckichData();
				setKarmelitanskiZakonSwieckich(data);
				setEditValues(data);
				setLoading(false);
			} catch (error) {
				console.error(
					'Error fetching KarmelitanskiZakonSwieckich data:',
					error
				);
				setLoading(false);
			}
		...
		fetchKarmelitanskiZakonSwieckich();
	}, []);
	...
	const handleSave = async () => {
		...
		try {
			const result = await saveKarmelitanskiZakonSwieckichData(editValues);
			if (result) {
				setKarmelitanskiZakonSwieckich(editValues);
				setIsEditing(false);
			} else {
	...
	if (!karmelitanskiZakonSwieckich) {
		return <div>Error loading KarmelitanskiZakonSwieckich data</div>;
	}
	...
	const hasRole = roles.some(
		(role) =>
			typeof role === 'string' && ['admin', 'karmelitanskizakon'].includes(role)
	);
					...
					<>
						...
						<div
							...
							dangerouslySetInnerHTML={{
								__html: karmelitanskiZakonSwieckich.content,
							}}
						></div>
