...
// Database
import findCaritasData from '@/actions/findCaritasData';
...
export default function Caritas() {
	const [caritas, setCaritas] = useState<CaritasProps | null>(null);
	...
	useEffect(() => {
		const fetchCaritas = async () => {
			try {
				const data = await findCaritasData();
				setCaritas(data);
				setEditValues(data);
				setLoading(false);
			} catch (error) {
				console.error('Error fetching Caritas data:', error);
				setLoading(false);
			}
		};
	...
	const handleSave = async () => {
		...
		try {
			const result = await saveCaritasData(editValues);
			if (result) {
				setCaritas(editValues);
				setIsEditing(false);
			} else {
	...
	if (!caritas) {
		return <div>Error loading Caritas data</div>;
	}
	...
	const hasRole = roles.some(
		(role) => typeof role === 'string' && ['admin', 'caritas'].includes(role)
	);
