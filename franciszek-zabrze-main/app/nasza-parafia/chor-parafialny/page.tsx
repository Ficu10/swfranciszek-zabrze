...
// Database
import findChorData from '@/actions/findChorData';
...
export default function Chor() {
	const [chor, setChor] = useState<ChorProps | null>(null);
	...
	useEffect(() => {
		const fetchChor = async () => {
			try {
				const data = await findChorData();
				setChor(data);
				setEditValues(data);
				setLoading(false);
			} catch (error) {
				console.error('Error fetching Chor data:', error);
				setLoading(false);
			}
		};
	...
	const handleSave = async () => {
		...
		try {
			const result = await saveChorData(editValues);
			if (result) {
				setChor(editValues);
				setIsEditing(false);
			} else {
	...
	if (!chor) {
		return <div>Error loading Chor data</div>;
	}
	...
	const hasRole = roles.some(
		(role) => typeof role === 'string' && ['admin', 'chor'].includes(role)
	);
