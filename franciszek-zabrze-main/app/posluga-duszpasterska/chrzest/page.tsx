...
// Database
import findChrzestData from '@/actions/findChrzestData';
import saveChrzestData from '@/actions/saveChrzestData';

interface ChrzestProps {
	content: string;
}

export default function Chrzest() {
	const [chrzest, setChrzest] = useState<ChrzestProps | null>(null);
	...
	useEffect(() => {
		const fetchChrzest = async () => {
			try {
				const data = await findChrzestData();
				setChrzest(data);
				setEditValues(data);
				setLoading(false);
			} catch (error) {
				console.error('Error fetching Chrzest data:', error);
				setLoading(false);
			}
		};
	...
	const handleSave = async () => {
		...
		try {
			const result = await saveChrzestData(editValues);
			if (result) {
				setChrzest(editValues);
				setIsEditing(false);
			} else {
	...
	if (!chrzest) {
		return <div>Error loading Chrzest data</div>;
	}
